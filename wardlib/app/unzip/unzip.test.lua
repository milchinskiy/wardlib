...
-- Tinytest suite for unzip module (init.lua)
--
...
return function(tinytest)
	local t = tinytest.new({ name = "unzip" })
	local MODULE_CANDIDATES = { "wardlib.app.unzip" }
	...
	local function load_module()
		...
		for _, name in ipairs(MODULE_CANDIDATES) do
			...
			if ok and type(mod) == "table" then
				t:ok(type(mod.Unzip) == "table", "module '" .. name .. "' did not return { Unzip = ... }")
				return mod
			end
			errs[#errs + 1] = name .. ": " .. tostring(mod)
		end
		t:ok(false, "failed to require unzip module. Tried:\n" .. table.concat(errs, "\n"))
	end
	...
	t:test("extract builds argv and does not include --", function()
		local mod = load_module()
		local Unzip = mod.Unzip
		...
		t:eq(calls.is_in_path[1], "unzip")
		t:deep_eq(last_cmd(), { "unzip", "-o", "a.zip", "x", "y", "-x", "*.tmp", "*.bak", "-d", "out" })
	end)

	t:test("list adds -l", function()
		local mod = load_module()
		local Unzip = mod.Unzip

		Unzip.list("a.zip", { quiet = true })
		t:deep_eq(last_cmd(), { "unzip", "-q", "-l", "a.zip" })
	end)
