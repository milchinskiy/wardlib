...
-- Tinytest suite for xz module (init.lua)
--
...
return function(tinytest)
	local t = tinytest.new({ name = "xz" })
	local MODULE_CANDIDATES = { "wardlib.app.xz" }
	...
	local function load_module()
		...
		for _, name in ipairs(MODULE_CANDIDATES) do
			...
			if ok and type(mod) == "table" then
				t:ok(type(mod.Xz) == "table", "module '" .. name .. "' did not return { Xz = ... }")
				return mod
			end
			errs[#errs + 1] = name .. ": " .. tostring(mod)
		end
		t:ok(false, "failed to require xz module. Tried:\n" .. table.concat(errs, "\n"))
	end
	...
	t:test("compress builds argv with level/extreme/threads and --", function()
		local mod = load_module()
		local Xz = mod.Xz

		Xz.compress("a", { level = 6, extreme = true, threads = 0 })
		t:eq(calls.is_in_path[1], "xz")
		t:deep_eq(last_cmd(), { "xz", "-e", "-6", "-T", "0", "--", "a" })
	end)

	t:test("decompress adds -d", function()
		local mod = load_module()
		local Xz = mod.Xz

		Xz.decompress("a.xz", { keep = true })
		t:deep_eq(last_cmd(), { "xz", "-d", "-k", "--", "a.xz" })
	end)
