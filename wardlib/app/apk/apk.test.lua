...
-- Tinytest suite for apk module (init.lua)
--
-- This suite mocks ward modules used by app.apk:
--   * ward.process (cmd)
--   * ward.env     (is_in_path)
--   * ward.fs      (is_exists, is_executable)

return function(tinytest)
	local t = tinytest.new({ name = "apk" })

	local MODULE_CANDIDATES = { "wardlib.app.apk" }
	...
	local function load_module()
		...
		for _, name in ipairs(MODULE_CANDIDATES) do
			...
			if ok and type(mod) == "table" then
				t:ok(type(mod.Apk) == "table", "module '" .. name .. "' did not return { Apk = ... }")
				return mod
			end
			errs[#errs + 1] = name .. ": " .. tostring(mod)
		end
		t:ok(false, "failed to require apk module. Tried:\n" .. table.concat(errs, "\n"))
	end
	...
	t:test("update forwards extra args", function()
		...
		Apk.update({ extra = { "--no-progress" } })
		t:deep_eq(last_cmd(), { "apk", "update", "--no-progress" })
	end)
