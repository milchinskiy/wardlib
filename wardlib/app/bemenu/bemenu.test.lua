...
-- Tinytest suite for bemenu module (init.lua)

return function(tinytest)
	local t = tinytest.new({ name = "bemenu" })

	local MODULE = "wardlib.app.bemenu"
	...
	local function load_module()
		...
		t:ok(ok and type(mod) == "table" and type(mod.Bemenu) == "table", "module did not return { Bemenu = ... }")
		return mod
	end
	...
	t:test("menu builds argv", function()
		local Bemenu = load_module().Bemenu
		Bemenu.menu({ ignorecase = true, center = true, lines = 12, prompt = "Run", extra = { "--no-overlap" } })
		t:deep_eq(last_cmd(), { "bemenu", "-i", "-c", "-l", "12", "-p", "Run", "--no-overlap" })
	end)

	t:test("run builds argv", function()
		local Bemenu = load_module().Bemenu
		Bemenu.run({ fork = true })
		t:deep_eq(last_cmd(), { "bemenu-run", "-f" })
	end)
	...
	t:test("bin validation: PATH vs absolute", function()
		...
		Bemenu.bin = "bemenu"
		env_ok = false
		local ok1 = pcall(function() Bemenu.menu(nil) end)
		t:falsy(ok1)
		t:eq(calls.is_in_path[#calls.is_in_path], "bemenu")

		reset()
		Bemenu = load_module().Bemenu
		Bemenu.bin = "/usr/bin/bemenu"
		fs_exists["/usr/bin/bemenu"] = true
		fs_exec["/usr/bin/bemenu"] = true
		local ok2 = pcall(function() Bemenu.menu(nil) end)
		t:truthy(ok2)
		t:eq(calls.is_exists[#calls.is_exists], "/usr/bin/bemenu")
		t:eq(calls.is_executable[#calls.is_executable], "/usr/bin/bemenu")
	end)
