...
-- Tinytest suite for blkid module (init.lua)
--
-- This suite mocks ward modules used by app.blkid:
--   * ward.process (cmd)
--   * ward.env     (is_in_path)
--   * ward.fs      (is_exists, is_executable)

return function(tinytest)
	local t = tinytest.new({ name = "blkid" })

	local MODULE_CANDIDATES = { "wardlib.app.blkid" }
	...
	local function load_module()
		...
		for _, name in ipairs(MODULE_CANDIDATES) do
			...
			if ok and type(mod) == "table" then
				t:ok(type(mod.Blkid) == "table", "module '" .. name .. "' did not return { Blkid = ... }")
				return mod
			end
			errs[#errs + 1] = name .. ": " .. tostring(mod)
		end
		t:ok(false, "failed to require blkid module. Tried:\n" .. table.concat(errs, "\n"))
	end
	...
	t:test("id supports output + tags + match + device", function()
		local mod = load_module()
		local Blkid = mod.Blkid
		...
		t:eq(calls.is_in_path[1], "blkid")
		t:deep_eq(last_cmd(), { "blkid", "-o", "export", "-s", "UUID", "-s", "TYPE", "-t", "TYPE=ext4", "/dev/sda1" })
	end)

	t:test("id supports multi-match + multi-devices", function()
		local mod = load_module()
		local Blkid = mod.Blkid

		Blkid.id({ "/dev/sda1", "/dev/sda2" }, {
			match = { "TYPE=ext4", "LABEL=myroot" },
			probe = true,
		})
		t:deep_eq(last_cmd(), { "blkid", "-p", "-t", "TYPE=ext4", "-t", "LABEL=myroot", "/dev/sda1", "/dev/sda2" })
	end)

	t:test("by_label and by_uuid are convenience forms", function()
		local mod = load_module()
		local Blkid = mod.Blkid

		Blkid.by_label("root")
		t:deep_eq(last_cmd(), { "blkid", "-L", "root" })

		Blkid.by_uuid("1111-2222")
		t:deep_eq(last_cmd(), { "blkid", "-U", "1111-2222" })
	end)
