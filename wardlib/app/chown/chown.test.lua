...
-- Tinytest suite for chown module (init.lua)

return function(tinytest)
	local t = tinytest.new({ name = "chown" })
	local MODULE = "wardlib.app.chown"
	...
	t:test("set builds argv with owner:group", function()
		local Chown = require(MODULE).Chown
		Chown.set({ "a" }, "root", "root", { recursive = true })
		t:deep_eq(last_cmd().argv, { "chown", "-R", "--", "root:root", "a" })
	end)

	t:test("set supports only group", function()
		local Chown = require(MODULE).Chown
		Chown.set({ "a" }, nil, "wheel", nil)
		t:deep_eq(last_cmd().argv, { "chown", "--", ":wheel", "a" })
	end)
