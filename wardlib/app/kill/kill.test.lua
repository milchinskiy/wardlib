...
-- Tinytest suite for kill module (init.lua)
--
-- This suite mocks ward modules used by app.kill:
--   * ward.process (cmd)
--   * ward.env     (is_in_path)
--   * ward.fs      (is_exists, is_executable)

return function(tinytest)
	local t = tinytest.new({ name = "kill" })

	local MODULE_CANDIDATES = { "wardlib.app.kill" }
	...
	local function load_module()
		...
		for _, name in ipairs(MODULE_CANDIDATES) do
			...
			if ok and type(mod) == "table" then
				t:ok(type(mod.Kill) == "table", "module '" .. name .. "' did not return { Kill = ... }")
				return mod
			end
			errs[#errs + 1] = name .. ": " .. tostring(mod)
		end
		t:ok(false, "failed to require kill module. Tried:\n" .. table.concat(errs, "\n"))
	end
	...
	t:test("kill with signal and pid", function()
		local mod = load_module()
		local Kill = mod.Kill

		Kill.kill(123, { signal = "TERM" })
		t:eq(calls.is_in_path[1], "kill")
		t:deep_eq(last_cmd(), { "kill", "-s", "TERM", "123" })
	end)

	t:test("killall with user + exact", function()
		local mod = load_module()
		local Kill = mod.Kill

		Kill.killall("firefox", { user = "alice", exact = true, signal = 9 })
		t:eq(calls.is_in_path[1], "killall")
		t:deep_eq(last_cmd(), { "killall", "-s", "9", "-e", "-u", "alice", "firefox" })
	end)

	t:test("pkill with -f and signal", function()
		local mod = load_module()
		local Kill = mod.Kill

		Kill.pkill("ssh", { full = true, signal = "KILL" })
		t:eq(calls.is_in_path[1], "pkill")
		t:deep_eq(last_cmd(), { "pkill", "-KILL", "-f", "ssh" })
	end)
