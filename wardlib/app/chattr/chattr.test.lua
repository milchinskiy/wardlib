...
-- Tinytest suite for chattr module (init.lua)

return function(tinytest)
	local t = tinytest.new({ name = "chattr" })
	local MODULE = "wardlib.app.chattr"
	...
	t:test("set builds argv with -- then mode", function()
		local Chattr = require(MODULE).Chattr
		Chattr.set({ "a", "b" }, "+i", { recursive = true })
		t:deep_eq(last_cmd().argv, { "chattr", "-R", "--", "+i", "a", "b" })
	end)
