...
-- Tinytest suite for feh module (init.lua)
--
-- This suite mocks ward modules used by app.feh:
--   * ward.process (cmd)
--   * ward.env     (is_in_path)
--   * ward.fs      (is_exists, is_executable)

return function(tinytest)
	local t = tinytest.new({ name = "feh" })

	local MODULE_CANDIDATES = { "wardlib.app.feh" }
	...
	local function load_module()
		...
		for _, name in ipairs(MODULE_CANDIDATES) do
			...
			if ok and type(mod) == "table" then
				t:ok(type(mod.Feh) == "table", "module '" .. name .. "' did not return { Feh = ... }")
				return mod
			end
			errs[#errs + 1] = name .. ": " .. tostring(mod)
		end
		t:ok(false, "failed to require feh module. Tried:\n" .. table.concat(errs, "\n"))
	end
	...
	t:test("view with fullscreen + randomize + inputs", function()
		local mod = load_module()
		local Feh = mod.Feh

		Feh.view({ "a.png", "b.png" }, { fullscreen = true, randomize = true })
		t:eq(calls.is_in_path[1], "feh")
		t:deep_eq(last_cmd(), { "feh", "-F", "--randomize", "a.png", "b.png" })
	end)

	t:test("bg with mode + no_fehbg", function()
		local mod = load_module()
		local Feh = mod.Feh

		Feh.bg("wall.png", { mode = "fill", no_fehbg = true })
		t:deep_eq(last_cmd(), { "feh", "--bg-fill", "--no-fehbg", "wall.png" })
	end)
