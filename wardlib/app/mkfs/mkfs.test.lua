...
-- Tinytest suite for mkfs module (init.lua)
--
-- This suite mocks ward modules used by app.mkfs:
--   * ward.process (cmd)
--   * ward.env     (is_in_path)
--   * ward.fs      (is_exists, is_executable)

return function(tinytest)
	local t = tinytest.new({ name = "mkfs" })

	local MODULE_CANDIDATES = { "wardlib.app.mkfs" }
	...
	local function load_module()
		...
		for _, name in ipairs(MODULE_CANDIDATES) do
			...
			if ok and type(mod) == "table" then
				t:ok(type(mod.Mkfs) == "table", "module '" .. name .. "' did not return { Mkfs = ... }")
				return mod
			end
	...
	t:test("prefers mkfs.<fstype> when available", function()
		env_in_path["mkfs.ext4"] = true

		local mod = load_module()
		mod.Mkfs.ext4("/dev/sda1")
		t:deep_eq(last_cmd(), { "mkfs.ext4", "/dev/sda1" })
	end)

	t:test("falls back to mkfs -t when mkfs.<fstype> not in PATH", function()
		env_in_path["mkfs"] = true

		local mod = load_module()
		mod.Mkfs.format("ext4", "/dev/sda1")
		t:deep_eq(last_cmd(), { "mkfs", "-t", "ext4", "/dev/sda1" })
	end)

	t:test("passes opts.extra before device", function()
		env_in_path["mkfs"] = true

		local mod = load_module()
		mod.Mkfs.format("xfs", "/dev/sdb1", { extra = { "-f" } })
		t:deep_eq(last_cmd(), { "mkfs", "-t", "xfs", "-f", "/dev/sdb1" })
	end)

	t:test("opts.bin supports absolute path", function()
		fs_exists["/usr/sbin/mkfs.ext4"] = true
		fs_exec["/usr/sbin/mkfs.ext4"] = true

		local mod = load_module()
		mod.Mkfs.ext4("/dev/sda1", { bin = "/usr/sbin/mkfs.ext4" })
		t:deep_eq(last_cmd(), { "/usr/sbin/mkfs.ext4", "/dev/sda1" })
	end)

	t:test("rejects flag-like fstype", function()
		env_in_path["mkfs"] = true
		local mod = load_module()
