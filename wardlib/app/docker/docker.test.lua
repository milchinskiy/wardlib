...
-- Tinytest suite for docker module (init.lua)
--
...
return function(tinytest)
	local t = tinytest.new({ name = "docker" })
	local MODULE = "wardlib.app.docker"
	...
	t:test("run builds argv with repeatable env/publish", function()
		local Docker = require(MODULE).Docker
		Docker.run("alpine:3", { "sh", "-lc", "echo ok" }, {
			rm = true,
			name = "t",
			env = { "A=1", "B=2" },
			publish = "8080:80",
	...
	t:test("ps adds filters", function()
		local Docker = require(MODULE).Docker
		Docker.ps({ all = true, filter = { "status=running", "label=a=b" } })
		t:deep_eq(last_cmd().argv, { "docker", "ps", "-a", "--filter", "status=running", "--filter", "label=a=b" })
	end)

	t:test("logs adds tail and since", function()
		local Docker = require(MODULE).Docker
		Docker.logs("c1", { follow = true, tail = 100, since = "10m" })
		t:deep_eq(last_cmd().argv, { "docker", "logs", "-f", "--since", "10m", "--tail", "100", "c1" })
	end)

	t:test("login supports password-stdin", function()
		local Docker = require(MODULE).Docker
		Docker.login("ghcr.io", { username = "u", password_stdin = true })
		t:deep_eq(last_cmd().argv, { "docker", "login", "-u", "u", "--password-stdin", "ghcr.io" })
	end)
