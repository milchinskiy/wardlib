...
-- Tinytest suite for wofi module (init.lua)

return function(tinytest)
	local t = tinytest.new({ name = "wofi" })

	local MODULE = "wardlib.app.wofi"
	...
	local function load_module()
		...
		t:ok(ok and type(mod) == "table" and type(mod.Wofi) == "table", "module did not return { Wofi = ... }")
		return mod
	end
	...
	t:test("dmenu builds argv with common options and --dmenu", function()
		local Wofi = load_module().Wofi
		Wofi.dmenu({
			conf = "wofi.conf",
			prompt = "Run",
			lines = 6,
	...
	t:test("show builds argv with --show and mode", function()
		local Wofi = load_module().Wofi
		Wofi.show("drun", { style = "style.css", width = "40%" })
		t:deep_eq(last_cmd(), {
			"wofi",
			"--style",
			"style.css",
	...
	t:test("bin validation: PATH vs absolute", function()
		...
		Wofi.bin = "wofi"
		env_ok = false
		local ok1 = pcall(function() Wofi.dmenu(nil) end)
		t:falsy(ok1)
		t:eq(calls.is_in_path[#calls.is_in_path], "wofi")

		-- absolute bin
		reset()
		Wofi = load_module().Wofi
		Wofi.bin = "/usr/bin/wofi"
		fs_exists["/usr/bin/wofi"] = true
		fs_exec["/usr/bin/wofi"] = true
		local ok2 = pcall(function() Wofi.dmenu(nil) end)
		t:truthy(ok2)
		t:eq(calls.is_exists[#calls.is_exists], "/usr/bin/wofi")
		t:eq(calls.is_executable[#calls.is_executable], "/usr/bin/wofi")
	end)
