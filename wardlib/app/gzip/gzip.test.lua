...
return function(tinytest)
	local t = tinytest.new({ name = "gzip" })
	local MODULE_CANDIDATES = { "wardlib.app.gzip" }
	...
	local function load_module()
		...
		for _, name in ipairs(MODULE_CANDIDATES) do
			...
			if ok and type(mod) == "table" then
				t:ok(type(mod.Gzip) == "table", "module '" .. name .. "' did not return { Gzip = ... }")
				return mod
			end
	...
	t:test("compress builds argv with level and --", function()
		local mod = load_module()
		local Gzip = mod.Gzip

		Gzip.compress({ "a", "b" }, { keep = true, level = 9 })
		t:eq(calls.is_in_path[1], "gzip")
		t:deep_eq(last_cmd(), { "gzip", "-k", "-9", "--", "a", "b" })
	end)
	...
	t:test("decompress adds -d", function()
		...
		Gzip.decompress("a.gz", { force = true })
		t:deep_eq(last_cmd(), { "gzip", "-d", "-f", "--", "a.gz" })
	end)
