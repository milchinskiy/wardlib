...
-- Tinytest suite for xargs module (init.lua)

return function(tinytest)
	local t = tinytest.new({ name = "xargs" })
	local MODULE = "wardlib.app.xargs"
	...
	t:test("run builds argv and uses -- before cmd", function()
		local Xargs = require(MODULE).Xargs
		Xargs.run({ "echo", "{}" }, { max_args = 10, verbose = true })
		t:deep_eq(last_cmd().argv, { "xargs", "-n", "10", "-t", "--", "echo", "{}" })
	end)
